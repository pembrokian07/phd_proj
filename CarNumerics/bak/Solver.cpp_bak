...
Solver::Solver(int m, int n, float theta1, float theta2):_m(m),_n(n),_theta1(theta1),_theta2(theta2)
{
    ...
    /*
    boost::push_back(_neumann_indices, boost::irange(0, (int) round(_n/4.0f-1)));
    boost::push_back(_neumann_indices, boost::irange((int)round(3.0f/4*_n), _n));
...
int Solver::p_j(int j, int i)
{
    ...
    {
        // map angle zero to 2*pi
        if(j==-1)
        {
            res = _neumann_indice_count-1;
        }
        // map 2*pi + d_a to d_a
        else if(j==_n)
        {
...
float Solver::neumann_bc(int j)
{
    float res;
    float alpha = (j+1)*_da;
    res = _m*_dr*(sinf(alpha)*cosf(alpha) + powf(cosf(alpha),2.0f) + 1);
...
void Solver::populate_neumann2()
{
    ...
    for(auto j: this->_neumann_indices)
    {
        ...
        (*_b)(k) = (3.0f*(sinf(alpha)+cosf(alpha))*powf(_dr, 2.0f)*cosf(alpha) - 2.0f*_dr*ita*neumann_bc(j));
        //(cosf(alpha)*3*powf(_dr, 2.0f)*(sinf(alpha)+cosf(alpha)) -  powf(_dr, 2.0f)*neumann_bc(j)*2*cosf(alpha)*beta);
            // -powf(_dr*cosf(alpha),2.0f)*df_dr(j) + _dr/_m*sinf(alpha)*cosf(alpha)*df_da(i, j) -  neumann_bc(j)*(1+1.0f/(2*_m));
            //-powf(_dr, 2.0f)*_theta1*(powf(cosf(alpha), 3.0f) + powf(sinf(alpha), 2.0f)*cosf(alpha));
    }
...
{
    float da2 = powf(_da, 2.0f);
    
    // i is 0
    for (int j=0; j<_n; j++)
    {
        (*_A)(j,j) = -(1.5f + 2.0f/da2);
        (*_A)(j,p_j(j+1)) = 1.0f/da2;
        (*_A)(j,p_j(j-1)) = 1.0f/da2;
        (*_A)(j, _n+j) = 1.5f*da2;
        
        float alpha = p_j(j+1)*_da;
        (*_b)(j) = -4.5f*powf(_dr, 2.0f)*(sinf(alpha)+cosf(alpha));
